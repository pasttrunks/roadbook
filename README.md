...
- Automatically look up original MSRP from key-free open vehicle data when the year, make, and model are supported.
- Compare MSRP, purchase price, and a make-aware planning estimate in one private ownership-value view.
- Ask NHTSA to decode modern VINs or identify older chassis numbers when its data supports them.
- Save comparable listings as evidence and track depreciation snapshots over time.
